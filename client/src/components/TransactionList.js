...
export const TransactionList = ({ transactions, setTransactions }) => {
  const removeTransaction = (_id) => {
    ...
    const deleteTransaction = async (_id) => {
      ...
      return await resp.status(204).json();
    };
          ...
          if (transaction.amount < 0) {
            return (
              ...
              >
                ...
                <li className="list-item minus-item">
                  <div>{text}</div>{" "}
                  <span>-${numberWithCommas(Math.abs(amount))}</span>
                </li>
              </div>
            );
          } else {
            return (
              ...
              >
                ...
                <li className="list-item plus-item">
                  <div>{text}</div> <span>${numberWithCommas(amount)}</span>
                </li>
              </div>
            );
          }
